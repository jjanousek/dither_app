...
builds `Ditherlab.app` into `~/Applications` (drag it to your Dock). Double-clicking it starts the server if needed and opens the app. Uses only tools that ship with macOS; re-run the script if you move the project folder. The dithered app icon is generated by `scripts/make-icon.py` — with the same Bayer matrix the app uses.
